...
# registry / codes 載入
# ------------------------------------------------------------------
# ⚠️ MoneyDJ 的 ?a= code 就是 PLB/PLZ 代碼本身（PLB22、PLB04…），
#    交接文件 §2「PLB22→AY01」那張對照表是錯的（AY/BQ code 在 MoneyDJ 抓不到資料）。
DEFAULT_SAMPLE = [
    {"fund_id": "barings_plb22", "moneydj_code": "PLB22",
     "display_name_zh": "霸菱優先順位資產抵押債-BM美元月配"},
    {"fund_id": "barings_plb04", "moneydj_code": "PLB04",
     "display_name_zh": "霸菱優先順位資產抵押債-G美元月配"},
    {"fund_id": "barings_plz45", "moneydj_code": "PLZ45",
     "display_name_zh": "霸菱成熟及新興高收益債-A美元月配"},
    {"fund_id": "barings_plz15", "moneydj_code": "PLZ15",
     "display_name_zh": "霸菱成熟及新興高收益債-A美元季配"},
]
    ...
    for dim, kws in DIMENSIONS.items():
        ...
        for fund in funds:
            ...
            for r in recs:
                ...
                if not r["field"]:
                    continue
                # 比對欄位名 + 表格標題 + 欄位值（部分維度名如「年化配息率%」
                # 出現在表頭那一列的「值」裡，故一併納入）
                hay = (r["field"] + " " + r["caption"] + " " + " ".join(r["values"])).lower()
